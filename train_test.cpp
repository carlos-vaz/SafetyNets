...
int main()
{
	...
	read_params(b0, HIDDEN, "trained_biases_0");
	read_params(b1, OUTPUT, "trained_biases_1");
...
//			cout << endl;
			...
			gradients0(out, desired, w1, img, z0, z1, Gw0_tmp, Gb0_tmp);
		...
		update(w0, Gw0, INPUT*HIDDEN);
		update(b0, Gb0, HIDDEN);
		update(w1, Gw1, HIDDEN*OUTPUT);
		update(b1, Gb1, OUTPUT);
		delete Gw0;
	...
	write_params(b0, HIDDEN, "trained_biases_0");
	write_params(w1, HIDDEN*OUTPUT, "trained_Bweights_1");
	write_params(b1, OUTPUT, "trained_biases_1");
